...
describe('Rooms Contract Conformance Tests (openapi.yaml)', () => {
  ...
  it('rejects unknown query parameters with 400 Problem Details', async () => {
    const response = await authorizedGet('/v1/rooms?unexpectedParam=true');
  ...
  it('returns 200 with single room representation matching contract schema', async () => {
    const response = await authorizedGet('/v1/rooms/rm_1a2B3cD');
  ...
  it('returns 400 for malformed ID with RFC 9457 Problem Details', async () => {
    const response = await authorizedGet('/v1/rooms/invalid_room_id!');
  ...
  it('returns 404 for non-existent room ID with RFC 9457 Problem Details', async () => {
    const response = await authorizedGet('/v1/rooms/rm_nonexistent999');
